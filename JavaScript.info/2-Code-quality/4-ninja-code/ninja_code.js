...
i = i ? i < o ? Math.max(0, len + i) : i : 0;

/**
 * Cool, right? If you write like that, a developer who comes across this line and 
 * tries to understand what is the value of i is going to have a merry time. 
 ...
 * Like 'a', 'b' or 'c'
 * 
 * A short variable disappears in the code like a real ninja in the forest. No one 
 * will be able to find it using “search” of the editor. And even if someone does, 
 * they won’t be able to “decipher” what the name a or b means.
 * 
 * …But there’s an exception. A real ninja will never use i as the counter in a "for" loop. 
 * Anywhere, but not here. Look around, there are many more exotic letters. 
 * For instance, 'x' or 'y'.
 * 
 * An exotic variable as a loop counter is especially cool if the loop body takes 1-2 pages 
 * (make it longer if you can). Then if someone looks deep inside the loop, 
 * they won’t be able to quickly figure out that the variable named 'x' is the loop counter.
 */
 ...
 * 
 *   …But what to do if data is already taken? Try value, it’s also universal. After 
 *   all, a variable eventually gets a value.
 * 
 ...
 *   for a ninja? Indeed, they are!
 * 
 *   Sure, the variable name still means something. It says what’s inside the variable: 
 *   a string, a number or something else. But when an outsider tries to understand 
 *   the code, they’ll be surprised to see that there’s actually no information at all! 
 *   And will ultimately fail to alter your well-thought code.
 *   
 ...
 * 
 * - **...But what if there are no more such name?
 ...
 * 
 * **One of the ways-use similar variable, like 'date' and 'data'.**
 * 
 * Mix them where you can.
 * 
 * A quick read of such code becomes impossible. And when there’s a typo… Ummm… 
 * We’re stuck for long, time to drink tea.
 */
 ...
 * screen – start it with 'display…', like 'displayMessage'. And then if another function
 * shows on the screen something else, like a user name, start it with 'show…' (like 'showName').
 * 
 ...
 * 'display'... in his code, then Peter could use 'render'.., and Ann – 'paint'.... 
 ...
 * 'printText(text)' will put the text on-screen. Let an unfamiliar reader think well
 * over similarly named function 'printMessage': “Where does it put the message? 
 * To a printer or on the screen?”. To make it really shine, 'printMessage(message)' 
...
/**
 * Add a new variable only when absolutely neccessary.
 * 
 ...
 * to analyze the code line-by-line and track the changes through every code branch.
 * 
...
/**
 * A fellow programmer who wants to work with elem in the second half of the function
 * will be surprised… Only during the debugging, after examining the code they will 
 * find out that they’re working with a clone!
 * 
...
/** 
 * Put underscores _ and __ before variable names. Like _name or __value. It would 
 * be great if only you knew their meaning. Or, better, add them just for fun, 
 * without particular meaning at all. Or different meanings in different places.
 * 
 ...
 * places. That makes the code even more fragile and increases the probability 
 * of future errors.
 */
 ...
 * the other hand – that brings no details. A reader may decide to look for a hidden 
 * meaning and meditate for an hour or two of their paid working time.
 */

// ======================== Overlap outer variables ========================//

// Use same names for variables inside and outside a function. As simple. 
// No efforts to invent new names.
 ...
 * 
 * For instance, a function 'validateEmail(email)' could (besides checking the email 
 * for correctness) show an error message and ask to re-enter the email.
 * 
 ...
 * - Follow many of them, and your code will become truly yours, no one 
 *   would want to change it.
 * - Follow all, and your code will become a valuable lesson for young 
 *   developers looking for enlightenment.
 */
